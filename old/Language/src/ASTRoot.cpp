#include <Script/Language/ASTRoot.hpp>
...
namespace Script::Language
{
	...
	ASTRoot::ASTRoot(std::string_view src)
	{
		initNativeParsers();
		parse(src);
	}
	...
	{
		auto pos = 0uz;
		auto context = AST::ParsingTools::Context{*this, *this, CppUtils::Language::Reader::Cursor{src, pos}, verbose};
	...
	[[nodiscard]] const CppUtils::Type::Token& ASTRoot::getReturnType() const
	{
		const auto mainFunctionSignature = AST::Function::FunctionSignature{"main", {}};
		if (!functionExists(mainFunctionSignature))
			throw std::runtime_error{"No entry point found: The main function is missing."};
