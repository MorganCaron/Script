...
namespace Script::Language::AST::Variable
{
	...
	{
		const auto& variableScope = dynamic_cast<const VariableScope&>(getParentScope().findScope(VariableScopeType));
		return variableScope.variableExists(getName());
	}

	void Variable::setValue(std::unique_ptr<Type::IValue>&& value)
	{
		auto& variableScope = dynamic_cast<VariableScope&>(getParentScope().findScope(VariableScopeType));
	...
	[[nodiscard]] const CppUtils::Type::Token& Variable::getReturnType() const
	{
		const auto &variableScope = dynamic_cast<const VariableScope&>(getParentScope().findScope(VariableScopeType));
