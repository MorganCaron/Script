...
{
	std::unique_ptr<AST::Core::Instruction> parseFunctionDeclaration(AST::ParsingTools::Context& context)
	{
		...
		auto functionName = std::string{cursor.getKeywordRequired("Le mot clef function doit etre suivi d un nom de fonction.")};
		if (cursor.getChar() != '(')
		...
		if (cursor.getChar() != ')')
		{
			...
			do
			{
				...
				context.skipSpacesAndComments();
				auto argumentName = std::string{cursor.getKeywordRequired("Le mot clef " + std::string{keyword} + " doit etre suivi d'un nom de variable.")};
				context.skipSpacesAndComments();
				if (cursor.getChar() != ':')
				...
				if (verbose)
				{
					CppUtils::Log::Logger::logInformation(std::string{keyword} + " " + argumentName + ": ", false);
					CppUtils::Log::Logger::logDetail(typeName, false);
				}
				auto argumentType = CppUtils::Type::Token{typeName};
		...
		if (cursor.getChar() == ':')
		{
			...
			auto returnType = CppUtils::Type::Token{typeName};
		...
		else
		{
			...
			functionDeclaration->setReturnType(AST::Type::Void::Token);
		}
