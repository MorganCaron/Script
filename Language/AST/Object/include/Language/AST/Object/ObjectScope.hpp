...
namespace Language::AST::Object
{
	...
	public:
		...
		inline void addClass(std::string_view name, std::unique_ptr<Type::Instance>&& classPrototype)
		{
			if (m_classes.find(name.data()) != m_classes.end())
		...
		[[nodiscard]] inline const std::unordered_map<std::string, std::unique_ptr<Type::Instance>>& getClasses() const noexcept
		{
			return m_classes;
		}
