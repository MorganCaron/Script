...
{
	VariableScope::VariableScope(BaseScope* scope, Scope::ScopeType scopeType):
	...
	void VariableScope::setVariable(std::string_view name, std::unique_ptr<Type::IValue>&& value)
	{
		if (m_variableSignatures.find(name.data()) != m_variableSignatures.end())
		{
	...
	const std::unique_ptr<Type::IValue>& VariableScope::getVariable(std::string_view name) const
	{
		if (m_variables.find(name.data()) != m_variables.end())
